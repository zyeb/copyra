...
class Search {
  ...
  func effectiveMode(for itemCount: Int, preferred mode: Mode = Defaults[.searchMode]) -> Mode {
    guard itemCount > Defaults[.largeHistoryThreshold] else {
      return mode
    }

    switch mode {
    case .fuzzy, .mixed, .regexp:
      return .exact
    default:
      return mode
    }
  ...
  func search(string: String, within: [Searchable]) -> [SearchResult] {
    ...
    switch effectiveMode(for: History.shared.totalCount) {
    case .mixed:
