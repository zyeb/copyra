...
class Clipboard {
  ...
  typealias SourceAppBundleProvider = () -> String?
  ...
  private let maxIgnoreRegexScanLength = 8_192

  private var ignoreRegexPatterns: [String] = []
  private var ignoreRegexCache: [NSRegularExpression] = []
  private var loggedInvalidRegexPatterns: Set<String> = []
  var sourceAppBundleProvider: SourceAppBundleProvider = {
  ...
  init() {
    ...
    rebuildIgnoreRegexCacheIfNeeded()
  }
  ...
  func copy(_ item: HistoryItem?, removeFormatting: Bool = false) {
    ...
    pasteboard.setString("", forType: .fromCopyra)
  ...
  func checkForChangesInPasteboard() { // swiftlint:disable:this cyclomatic_complexity
    ...
    if pasteboard.pasteboardItems?.contains(where: { $0.types.contains(.fromCopyra) }) != true {
    ...
    let sourceAppBundle = sourceAppBundleProvider()
  ...
  private func shouldIgnore(_ item: NSPasteboardItem) -> Bool {
    ...
    let scannedString = String(string.prefix(maxIgnoreRegexScanLength))
    let searchRange = NSRange(scannedString.startIndex..., in: scannedString)
    for regex in ignoreRegexCache {
      if regex.numberOfMatches(in: scannedString, range: searchRange) > 0 {
        return true
      }
    }
  ...
  private func rebuildIgnoreRegexCacheIfNeeded() {
    let patterns = Defaults[.ignoreRegexp]
    guard patterns != ignoreRegexPatterns else {
      return
    }

    ignoreRegexPatterns = patterns
    ignoreRegexCache = []
    loggedInvalidRegexPatterns = []

    for pattern in patterns {
      do {
        let regex = try NSRegularExpression(pattern: pattern)
        ignoreRegexCache.append(regex)
      } catch {
        if !loggedInvalidRegexPatterns.contains(pattern) {
          NSLog("Ignoring invalid regexp '%@': %@", pattern, String(describing: error))
          loggedInvalidRegexPatterns.insert(pattern)
        }
      }
    }
  }
