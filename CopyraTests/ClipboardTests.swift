...
class ClipboardTests: XCTestCase {
  ...
  let customType = NSPasteboard.PasteboardType(rawValue: "org.copyra.ConfidentialType")
  ...
  override func setUp() {
    super.setUp()
    clipboard.resetObserverState()
    clipboard.sourceAppBundleProvider = { "com.apple.finder" }
  ...
  override func tearDown() {
    ...
    clipboard.sourceAppBundleProvider = { NSWorkspace.shared.frontmostApplication?.bundleIdentifier }
    clipboard.resetObserverState()
  }
  ...
  func testIgnoreOnlyNextEventIsEnabled() {
    ...
    let ignoredHookExpectation = expectation(description: "First hook is ignored")
    ignoredHookExpectation.isInverted = true
    clipboard.onNewCopy({ (_: HistoryItem) in
      ignoredHookExpectation.fulfill()
    })
    ...
    let resumedHookExpectation = expectation(description: "Hook resumes after first ignored event")
    clipboard.onNewCopy({ (_: HistoryItem) in
      resumedHookExpectation.fulfill()
    })
    pasteboard.declareTypes([.string], owner: nil)
    pasteboard.setString("bar", forType: .string)
    clipboard.checkForChangesInPasteboard()
  ...
  @MainActor
  func testInvalidIgnoreRegexDoesNotDisableValidPatterns() {
  ...
  func testCopy() {
    ...
    XCTAssertEqual(pasteboard.string(forType: .fromCopyra), "")
  ...
  func testRemovesDisabledTypes() {
    ...
    let hookExpectation = expectation(description: "Hook is called")
    clipboard.onNewCopy({ (item: HistoryItem) in
      XCTAssertEqual(Set(item.contents.map({ $0.type })), Set([self.fileURLType.rawValue]))
      hookExpectation.fulfill()
    })
